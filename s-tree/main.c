...
void st_remove(struct st_node **root, struct st_node *del)
{
    if (st_right(del)) {
        ...
        st_replace_right(del, least);       // Mask - AAAA => st_replace_right(del, least);
        st_update(root, st_right(least));   // Mask - BBBB => st_update(root, st_right(least));
        return;
    }
    ...
    if (st_left(del)) {
        ...
        st_replace_left(del, most);         // Mask - CCCC => st_replace_left(del, most);
        st_update(root, st_left(most));     // Mask - DDDD => st_update(root, st_left(most));
        return;
    }
    ...
    st_update(root, parent);    // Mask - EEEE => st_update(root, parent);
}
...
static void __treeint_dump(struct st_node *n, int depth)
{
    ...
    __treeint_dump(st_left(n), depth + 1);  // Mask - FFFF => st_left(n)

    struct treeint *v = treeint_entry(n);
    printf("%d\n", v->value);

    __treeint_dump(st_right(n), depth + 1); // Mask - GGGG => st_right(n)
}
