...
static struct qsort *allocate_thread(struct common *c)
{
    ...
    for (int i = 0; i < c->nthreads; i++)
        if (c->pool[i].st == ts_idle) {
            c->idlethreads--;
            // Mask - Remove => c->pool[i].st = ts_work;
            verify(pthread_mutex_lock(&c->pool[i].mtx_st));
            c->pool[i].st = ts_work;    // Mask - Create => c->pool[i].st = ts_work;
...
again:
    ...
    verify(pthread_mutex_lock(&qs->mtx_st));
    while (qs->st == ts_idle)
        verify(pthread_cond_wait(&qs->cond_st, &qs->mtx_st));   // Mask - HHHH => pthread_cond_wait(&qs->cond_st, &qs->mtx_st)
    verify(pthread_mutex_unlock(&qs->mtx_st));
    if (qs->st == ts_term) {
        return NULL;
    }
    ...
    if (c->idlethreads == c->nthreads) {
        for (i = 0; i < c->nthreads; i++) {
            ...
            qs2->st = ts_term;
            verify(pthread_cond_signal(&qs2->cond_st)); // Mask - JJJJ => pthread_cond_signal(&qs2->cond_st)
            verify(pthread_mutex_unlock(&qs2->mtx_st));
        }
        verify(pthread_mutex_unlock(&c->mtx_al));
        return NULL;
