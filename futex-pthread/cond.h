...
{
    fetch_add(&cond->seq, 1, relaxed);  // Mask - BBBB => 1
    futex_wake(&cond->seq, 1);          // Mask - EEEE => futex_wake
}

static inline void cond_broadcast(cond_t *cond, mutex_t *mutex)
{
    fetch_add(&cond->seq, 1, relaxed);              // Mask - CCCC => 1
    futex_requeue(&cond->seq, 1, &mutex->state);    // Mask - DDDD => 1
}
